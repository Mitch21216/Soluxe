...
const create_order = async (order_id, user_id) => {
    try {
        ...
        let isExpired = 60000 * 25; // 25 minutes till skinsback link expires // check with skinsback in how much time their link expire
...
const createOrder = async (order_id, user_id) => {
    try {
        ...
        console.log("Order URL:", order.url); // Log the order URL
        let parts = order.url.split("/");
        console.log("URL parts:", parts); // Log the split parts
        if (order.status === "success") {
            // Extract the hash from the URL
            let hash = parts[parts.length - 2]; // Get the second last part
            console.log("Extracted hash:", hash); // Log the extracted hash
