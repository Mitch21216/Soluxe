  ...
  io.of("/chat").on("connection", socket => {
    ...
    socket.on("send-chat-message", async content => {
      ...
      try {
        ...
        if (dbUser.rank >= 5 && command.includes(".remove-message")) {
        ...
        if (dbUser.rank >= 5 && command.includes(".pause-chat")) {
        ...
        if (dbUser.rank >= 5 && command.includes(".ban-user")) {
          // Validate admin input
          if (!args[1])
        ...
        if (dbUser.rank >= 5 && command.includes(".mute-user")) {
          // Validate admin input
          if (!args[1])
            return socket.emit("notify-error", "Please specify <UserId>");
