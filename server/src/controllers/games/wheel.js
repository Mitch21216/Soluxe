...
const getWinningColor = async winningMultiplier => {
  return new Promise((resolve, reject) => {
    if (winningMultiplier === 1) {
      ...
      resolve("purple");
    } else if (winningMultiplier === 4) {
      resolve("green");
    } else if (winningMultiplier === 9) {
      resolve("blue");
...
const listen = io => {
  ...
  const endCurrentWheelGame = async () => {
    ...
    try {
      ...
      const winningMultiplier = WHEEL_ORDER[winningIndex];
        ...
        if (winningMultiplier === 0) {
          ...
          setTimeout(async () => {
            ...
            const secondMultiplier =
              WHEEL_ORDER[Math.floor(secondRandomData.module)] !== 0
                ? WHEEL_ORDER[Math.floor(secondRandomData.module)]
                : 1;
  ...
  io.of("/wheel").on("connection", socket => {
    ...
    socket.on("join-game", async (color, betAmount) => {
      ...
      if (
        typeof color !== "string" ||
        !["red", "blue", "grey", "green", "purple", "pink"].includes(color)
      )
