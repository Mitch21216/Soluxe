...
const listen = async (io) => {
  ...
  const generateCaseResult = async (caseObj, hash, playerCount, roundNum, players) => {
    const caseInfo = caseList.get(caseObj.slug);
    if (!caseInfo) {
  ...
  io.of("/battles").on("connection", socket => {
    ...
    socket.on("battles:join", async (battleId) => {
      try {
        ...
        if (pendingGame && pendingGame.cancelling) return socket.emit("battles:error", "This game is in cancellation process, you cannot join anymore!")
    ...
    socket.on('battles:cancel', async (battleId) => {
      try {
        if (!loggedIn) return socket.emit("battles:error", "You are not logged in!");

        const battle = PENDING_GAMES.find(game => String(game._id) === battleId) ? PENDING_GAMES.find(game => String(game._id) === battleId) : await BattlesGame.findOne({ _id: battleId });
        if (!battle) return socket.emit("battles:error", "The game you are trying to cancel is invalid!");

        if (String(battle._creator) != user.id) return socket.emit(
          "battles:error",
          "To cancel the battle you must be the creator!"
        );

        if (battle.status !== 1) return socket.emit("battles:error", "You can only cancel the game when no-one joined!")

        const pendingGameIndex = PENDING_GAMES.findIndex(game => String(game._id) === String(battleId))
        PENDING_GAMES.splice(pendingGameIndex, 1)

        const betAmount = parseFloat(battle.betAmount)
        ...
        insertNewWalletTransaction(user.id, Math.abs(parseFloat(betAmount.toFixed(2))), 'Battle cancellation', { battlesGameId: battle._id });

        // removing wagered from rakeback
        addWageredToRakeBack(user.id, -Math.abs(parseFloat(betAmount.toFixed(2))))

        io.of("/battles").to(user.id).emit("update-wallet", Math.abs(parseFloat(betAmount.toFixed(2))));
        io.of("/battles").to("battles").emit("battles:cancelled", battle._id);

        await BattlesGame.deleteOne({ _id: battle._id })
      } catch (e) {
        console.error(e);

        return socket.emit(
          "battles:error",
          "There was an error while cancelling the battle"
        );
      }
    })
    ...
    socket.on("battles:callbot", async (battleId) => {
      try {
        ...
        const bot1 = {
          id: "bot1",
          username: "Crusader",
          pfp: "https://i.imgur.com/ZOS8kd3.png"
        };
